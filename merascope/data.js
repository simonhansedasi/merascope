...
(function () {
  ...
  var INDICATORS = [
    { k: 'transmission', label: 'Transmission proximity', def: 40, icon: 'pylon',
      ...
      source: 'EIA Form 860 + OSM', method: 'Centroid-to-line distance (UTM)', freq: 'Annual (EIA 860)',
      confidence: 'High', confidence_note: 'Comprehensive national line network; no interpolation.' },
    { k: 'water', label: 'Water availability', def: 35, icon: 'droplet',
      score_col: 'water_score', nat_col: 'water_score_nat',
      source: 'PRISM Climate Group', method: 'Nearest-pixel raster lookup', freq: '30-yr normals (updated decennially)',
      confidence: 'High', confidence_note: 'Continuous national raster; full spatial coverage.' },
    { k: 'community', label: 'Community burden', def: 25, icon: 'rings',
      score_col: 'ej_score', nat_col: 'ej_score_nat',
      source: 'Census ACS 5-yr', method: 'ZCTA-direct (poverty + minority rate)', freq: 'Annual (ACS release)',
      confidence: 'High', confidence_note: 'Direct ZCTA attribute; no spatial interpolation.' },
    { k: 'seismic', label: 'Seismic safety', def: 0, icon: 'wave',
      score_col: 'seismic_score', nat_col: 'seismic_score_nat',
      source: 'USGS ASCE 7-22', method: 'IDW from 60-pt sample per state', freq: 'Static (hazard model updates ~5 yr)',
      confidence: 'Medium', confidence_note: 'IDW from 60 sample points per state; sharp fault-zone gradients may be smoothed.' },
    { k: 'flood', label: 'Flood safety', def: 0, icon: 'flood',
      score_col: 'flood_score', nat_col: 'flood_score_nat',
      source: 'FEMA NFHL', method: 'Point-in-polygon (hard gate)', freq: 'Continuous (FEMA updates by county)',
      confidence: 'High', confidence_note: 'Binary hard gate from comprehensive national flood hazard layer.' },
    { k: 'contamination', label: 'Contamination distance', def: 0, icon: 'borehole',
      score_col: 'contamination_score', nat_col: 'contamination_score_nat',
      source: 'EPA Toxics Release Inventory', method: 'Centroid-to-point distance (UTM)', freq: 'Annual (TRI reporting year)',
      confidence: 'High', confidence_note: 'Federally mandated reporting; comprehensive national coverage.' },
    { k: 'waterway', label: 'Waterway sensitivity', def: 0, icon: 'river',
      score_col: 'waterway_score', nat_col: 'waterway_score_nat',
      source: 'OpenStreetMap', method: 'Centroid-to-line distance (UTM)', freq: 'Static (OSM snapshot)',
      confidence: 'High', confidence_note: 'Dense OSM line coverage; minor gaps in very rural areas.' },
    { k: 'geothermal', label: 'Geothermal opportunity', def: 0, icon: 'thermal',
      score_col: 'geothermal_score', nat_col: 'geothermal_score_nat',
      source: 'IHFC GHFDB 2024', method: 'IDW (k=8, p=2) from borehole measurements', freq: 'Static (2024 release)',
      confidence: 'Low', confidence_note: 'Borehole measurements very sparse outside volcanic/geothermal hotspots; IDW artifacts likely in data-poor regions.' },
    { k: 'flatness', label: 'Terrain flatness', def: 0, icon: 'contour',
      score_col: 'flatness_score', nat_col: 'flatness_score_nat',
      source: 'NASA SRTM1 (AWS S3)', method: 'np.gradient slope < 5 deg flat fraction', freq: 'Static (SRTM 2000)',
      confidence: 'Medium', confidence_note: 'SRTM vintage 2000; land cover changes since then not reflected.' },
    { k: 'aquifer', label: 'Aquifer depth', def: 0, icon: 'borehole',
      score_col: 'aquifer_score', nat_col: 'aquifer_score_nat',
      source: 'USGS NWIS', method: 'IDW (k=8, p=2) from well measurements', freq: 'Annual (NWIS field records)',
      confidence: 'Low', confidence_note: 'Well records extremely sparse in arid West (NV, NM, WY, AZ); IDW from widely-spaced wells is unreliable.' },
    { k: 'soil', label: 'Soil suitability', def: 0, icon: 'parcel',
      score_col: 'soil_score', nat_col: 'soil_score_nat',
      source: 'USDA SSURGO (SDM API)', method: 'IDW from map-unit centroids (HSG A-D)', freq: 'Static (SSURGO vintage)',
      confidence: 'Medium', confidence_note: 'Good coverage in agricultural Midwest; sparser in western rangelands and mountains.' },
    { k: 'slope', label: 'Slope suitability', def: 0, icon: 'wave',
      score_col: 'slope_score', nat_col: 'slope_score_nat',
      source: 'NASA SRTM1 (AWS S3)', method: 'Mean slope (degrees) within ZCTA bounds', freq: 'Static (SRTM 2000)',
      confidence: 'Medium', confidence_note: 'SRTM vintage 2000; averaged over ZCTA bounds may mask sub-ZCTA terrain variation.' },
    { k: 'pop_exposure', label: 'Population exposure', def: 0, icon: 'pin',
      score_col: 'pop_exposure_score', nat_col: 'pop_exposure_score_nat',
      source: 'Census ACS 5-yr', method: 'ZCTA-direct (population density)', freq: 'Annual (ACS release)',
      confidence: 'High', confidence_note: 'Direct ZCTA attribute; no spatial interpolation.' },
    { k: 'soil_profile', label: 'Soil profile chemistry', def: 0, icon: 'plumb',
      score_col: 'soil_profile_score', nat_col: 'soil_profile_score_nat',
      source: 'USDA SSURGO (SDM API)', method: 'Depth-weighted horizon composite; IDW', freq: 'Static (SSURGO vintage)',
      confidence: 'Medium', confidence_note: 'Good coverage in agricultural Midwest; sparser in western rangelands and mountains.' },
    { k: 'ksat', label: 'Hydraulic K-sat', def: 0, icon: 'borehole',
      score_col: 'ksat_score', nat_col: 'ksat_score_nat',
      source: 'USDA SSURGO (SDM API)', method: 'Thickness-weighted mean K-sat to 150 cm', freq: 'Static (SSURGO vintage)',
      confidence: 'Medium', confidence_note: 'Good coverage in agricultural Midwest; sparser in western rangelands and mountains.' },
    { k: 'substation', label: 'Substation proximity', def: 0, icon: 'pylon',
      score_col: 'substation_score', nat_col: 'substation_score_nat',
      source: 'EIA Form 860', method: 'Centroid-to-point distance + capacity weight', freq: 'Annual (EIA 860)',
      confidence: 'High', confidence_note: 'Federally reported; comprehensive national substation inventory.' },
    { k: 'superfund', label: 'Superfund distance', def: 0, icon: 'borehole',
      score_col: 'superfund_score', nat_col: 'superfund_score_nat',
      source: 'EPA Envirofacts NPL', method: 'Centroid-to-point distance (UTM)', freq: 'Continuous (NPL updates)',
      confidence: 'High', confidence_note: 'Federally maintained NPL; comprehensive national coverage.' },
    { k: 'rcra', label: 'RCRA site distance', def: 0, icon: 'borehole',
      score_col: 'rcra_score', nat_col: 'rcra_score_nat',
      source: 'EPA Envirofacts RCRA', method: 'Centroid-to-point distance (UTM)', freq: 'Annual (RCRA reporting)',
      confidence: 'High', confidence_note: 'Federally mandated reporting; comprehensive national coverage.' },
    { k: 'air_quality', label: 'Air quality (NAAQS)', def: 0, icon: 'wave',
      score_col: 'air_quality_score', nat_col: 'air_quality_score_nat',
      source: 'EPA Green Book', method: 'Point-in-polygon non-attainment areas', freq: 'Continuous (EPA designations)',
      confidence: 'High', confidence_note: 'Binary attainment designation from comprehensive national regulatory dataset.' },
    { k: 'fiber', label: 'Fiber connectivity', def: 0, icon: 'pylon',
      score_col: 'fiber_score', nat_col: 'fiber_score_nat',
      source: 'PeeringDB /api/fac', method: 'Centroid-to-point distance (carrier hotels)', freq: 'Static (PeeringDB snapshot)',
      confidence: 'Medium', confidence_note: 'Carrier hotel data only; dark fiber and enterprise lit fiber not captured.' },
    { k: 'water_stress', label: 'Water stress', def: 0, icon: 'droplet',
      score_col: 'water_stress_score', nat_col: 'water_stress_score_nat',
      source: 'WRI Aqueduct 3.0', method: 'Watershed spatial join (HydroBASINS L6)', freq: 'Static (2023 release)',
      confidence: 'Medium', confidence_note: 'Watershed-level granularity (HydroBASINS L6) is coarser than ZCTA; score represents the containing watershed, not the ZCTA.' },
    { k: 'grid_capacity', label: 'Grid capacity', def: 0, icon: 'pylon',
      score_col: 'grid_capacity_score', nat_col: 'grid_capacity_score_nat',
